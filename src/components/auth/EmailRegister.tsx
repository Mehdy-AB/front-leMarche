...
export const EmailRegister=({etape,setEtape,setEmail}:{
    ...
    const onSubmit = (data: SendCodeDto) => {
      axiosGhost.post('/user/auth/email-code',data).then(res=>{
        if(res.data.error){
        ...
        setError('email', { message: e.response?.data?.message || 'Email is invalide' })
      })
    }

    return(<>
    <form className="flex flex-col w-full" onSubmit={handleSubmit(onSubmit)}>
    <span className="mt-10 text-lg font-semibold">Quelle est votre adresse e-mail&nbsp;?</span>
    <input
      type="text"
      {...register("email")}
    ...
    <button disabled={isSubmitting} type="submit" className="bg-orange-400 disabled:bg-gray-200 rounded-4xl py-3 rounded-full mt-6 font-semibold text-white">
      {isSubmitting?<Loader/>:'Suivant'}
        ...
        {etape===0&&<><div className="grid grid-cols-11 items-center mt-16">
          ...
          <span className="w-full col-span-1 text-center text-xl font-semibold">OU</span>
          <span className="w-full col-span-5 rounded bg-gray-200 h-[2px]"></span>
        </div>
