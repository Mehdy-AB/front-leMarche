...
import {  OneAttribute } from '@/lib/types/types';
...
type Props = {
  ...
  index: number;
  field: any;
  originalAttr: OneAttribute | undefined;
};
  ...
  return (
    <div className={`flex flex-col ${index % 2 === 0 ? 'border-r-2 pr-2' : 'border-l-2 pl-2'}`}>
      ...
      {originalAttr?.type !== 'SELECT' ? (
        <SingleInput
          placeholder=""
          max={originalAttr?.max ??undefined}
          min={originalAttr?.min ??undefined}
          step={originalAttr?.step ??undefined}
          index={index}
          type="number"
