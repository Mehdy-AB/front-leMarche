    ...
    return(
                <div onClick={() => router.push(`/ad/${ad.id}`)} className="border border-gray-200 hover:bg-gray-100 cursor-pointer relative rounded-xl shadow-sm hover:shadow-md transition overflow-hidden">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" onClick={(e)=>{
                    e.stopPropagation();
                    addfavoriteAd(ad.id,!!ad.favoritesBy.find(f=>f.id===session.data?.user.id))}} strokeWidth={1.5} stroke="currentColor" className={`size-7 text-colorOne cursor-pointer hover:fill-colorOne absolute ${ad.favoritesBy.find(f=>f.id===session.data?.user.id)?'fill-colorOne':''} top-2 right-2`}>
